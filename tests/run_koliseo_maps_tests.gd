...
func _test_alive_grade() -> void:
	...
	eq(float(quiet["jewel_mix"]), 0.0, "an unknown map does not stain a jewel")
	truthy(float(life.grade_for("windmere", "ground", 0, Vector2i(1, 1))["jewel_mix"]) > 0.5, "Windmere stains each diamond")
	var jewel_a: Color = life.jewel_for("windmere", Vector2i(0, 0))
	var jewel_b: Color = life.jewel_for("windmere", Vector2i(1, 0))
	truthy(jewel_a != jewel_b, "neighboring Windmere cells take different jewels")
	truthy(jewel_a.r > jewel_a.b, "Windmere keeps a rare gold tile")
	var cool: Color = life.jewel_for("windmere", Vector2i(1, 0))
	truthy(cool.b > cool.r, "the other Windmere jewels stay cyan")
	eq(life.GRID_INK_PX >= 3.0, true, "the tactical grid line is thick enough to read")
	var life_src := FileAccess.get_file_as_string("res://board/koliseo_life.gd")
	truthy(life_src.contains("paint_backdrop"), "ship arenas paint a backdrop behind the diamonds")
