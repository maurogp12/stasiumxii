...
func _test_advance_manhattan_range_gate() -> void:
	...
	var hud := FileAccess.get_file_as_string("res://ui/hud.gd")
	eq(hud.contains("Chebyshev"), false, "HUD selected label never names Chebyshev")
	truthy(hud.contains("range_caption"), "selected label uses shared range chrome")
	var tooltip := FileAccess.get_file_as_string("res://data/spell_tooltip.gd")
	truthy(tooltip.contains("Manhattan"), "range chrome still names Manhattan for Advance")
...
func _test_preview_cast() -> void:
	...
	eq(str(preview["notes"]).contains("Chebyshev"), false, "Shoulder preview notes are player range language")
	...
	eq(str(preview["on_connect_text"]).contains("Chebyshev"), false, "Advance connect kit line is player range language")
...
func _test_spell_tooltip_cards() -> void:
	...
	truthy(hud._selected_label.text.contains("range 2–5"), "Mark Shot selected line uses range language")
	eq(hud._selected_label.text.contains("Chebyshev"), false, "Mark Shot selected line does not name Chebyshev")
	...
	hud._on_spell_pressed(SpellKits.ADVANCE)
	truthy(hud._selected_label.text.contains("range 1–2 Manhattan"), "Advance selected line names Manhattan range")
	eq(hud._selected_label.text.contains("Chebyshev"), false, "Advance selected line does not name Chebyshev")
	...
	eq(tooltip_src.contains("Chebyshev"), false, "tooltip chrome never names Chebyshev")
	...
	eq(hud_src.contains("Chebyshev"), false, "HUD chrome never names Chebyshev")
	...
	eq(readme.contains("range 2–5 Chebyshev"), false, "README play copy drops Chebyshev from range")
	truthy(readme.contains("range 2–5"), "README play copy uses range language")
