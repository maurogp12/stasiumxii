...
func _test_deploy_rejects_lava() -> void:
	_sim.reset_match({
		"seed": 1,
		"tiles": [{"pos": Vector2i(0, 3), "terrain": "lava", "elevation": 0.0}],
	})
	eq(_sim.snapshot()["phase"], "DEPLOYMENT", "live reset still starts in DEPLOYMENT")
	eq(_sim.can_place(0, Vector2i(0, 3))["reason"], "not_walkable", "lava ring cell is not_walkable")
	eq(_sim.place_unit(0, Vector2i(0, 3))["reason"], "not_walkable", "place onto lava is rejected")
	eq(_unit(0)["placed"], false, "failed lava place leaves Kestrel unplaced")
	eq(_sim.legal_deploy_cells(0).has(Vector2i(0, 3)), false, "legal deploy cells omit lava")
	eq(_sim.legal_deploy_cells(0).size(), 13, "S+W ring drops the lava cell")
	eq(_sim.place_unit(0, Vector2i(0, 2))["ok"], true, "adjacent Ground ring cell still places")
	eq(_sim.place_unit(1, Vector2i(7, 3))["ok"], true, "P2 still places on Ground")
