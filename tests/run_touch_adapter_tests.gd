...
func _test_mobile_target_pick() -> void:
	...
	var width_fit := (960.0 - 16.0) / 960.0
	eq(portrait > width_fit, true, "phone portrait is larger than the full-width fit")
	var play_h := 1400.0 - TOUCH.PLAY_TOP - (TOUCH.VIEW_H - TOUCH.PLAY_BOTTOM)
	var cover := 960.0 / 960.0 + play_h / 500.0
	near(portrait, cover, "portrait diamond covers the clear play rectangle")
	eq(portrait > TOUCH.MOBILE_CELL_TARGET_PX / TOUCH.DIAMOND_H, true, "portrait cells are larger than a 44px diamond")
	eq(TOUCH.DIAMOND_H * portrait > 90.0, true, "portrait diamond is tall enough to tap")
	eq(portrait <= TOUCH.MOBILE_BOARD_ZOOM_MAX, true, "portrait zoom stays inside the mobile cap")
	var metric := (960.0 / portrait) / 960.0 + (play_h / portrait) / 500.0
	near(metric, 1.0, "play-rect corners sit on the iso diamond")
	var room := TOUCH.pan_room(960.0, 500.0, Vector2(960, 1400), portrait, true)
	eq(room.x > 200.0, true, "the phone zoom leaves sideways pan room")
	eq(room.y > 40.0, true, "the phone zoom leaves vertical pan room")
	...
	var wide := TOUCH.board_zoom(960.0, 500.0, Vector2(1600, 720), true)
	var wide_h := TOUCH.PLAY_BOTTOM - TOUCH.PLAY_TOP
	near(wide, 1600.0 / 960.0 + wide_h / 500.0, "a short phone window still fills the clear band")
	eq(wide > desktop_zoom, true, "a short phone window zooms in past the desktop fit")
	var tall := TOUCH.board_zoom(960.0, 500.0, Vector2(960, 2000), true)
	eq(tall > portrait, true, "a taller phone zooms further in")
	eq(tall < TOUCH.MOBILE_BOARD_ZOOM_MAX, true, "a 2000px phone is still under the cap")
	eq(TOUCH.board_zoom(960.0, 500.0, Vector2(960, 3000), true), TOUCH.MOBILE_BOARD_ZOOM_MAX, "an extreme phone hits the zoom cap")
	var pawn := Pawn.new()
...
func _test_sources_keep_desktop_and_hub() -> void:
	...
	truthy(project.contains("window/handheld/orientation=1"), "phones open in portrait")
