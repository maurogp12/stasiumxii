...
func _test_mobile_target_pick() -> void:
	...
	# 20:9 landscape phone. Godot expand keeps height 720 and widens the canvas.
	var phone_view := Vector2(1600, 720)
	var phone_band := TOUCH.play_band_for(phone_view, true)
	eq(phone_band.x, TOUCH.MOBILE_FRAME_TOP, "phone frame keeps a slim top inset")
	...
	eq(phone < 2.0, true, "a landscape phone stays under the 0.1.21 cover zoom")
	var phone_span_y := phone_band.y - phone_band.x
	var phone_vis_h := phone_span_y / phone
	var phone_vis_w := phone_view.x / phone
	eq(phone_vis_h / 500.0 >= 0.8, true, "most of the diamond height is on screen")
	eq(phone_vis_w >= 960.0, true, "the diamond width fits, so the side gutter stays modest")
	eq(TOUCH.DIAMOND_H * phone >= 40.0, true, "a landscape diamond stays fat enough to tap")
	eq(TOUCH.DIAMOND_H * phone <= 60.0, true, "a landscape diamond is not a giant cover crop")
	near(phone, 1.47619, "20:9 phone overview zoom")
	var room := TOUCH.pan_room(960.0, 500.0, phone_view, phone, true)
	eq(room.x < 1.0, true, "the width fits, so sideways pan has no gutter to hide")
	eq(room.y > 20.0 and room.y < 80.0, true, "only the diamond tips sit past the frame")
	...
	eq(focused.y, 0.0, "a level focus does not shift vertically")
	eq(TOUCH.drag_is_pan(Vector2.ZERO, Vector2(10, 4), false), false, "a short finger slide still picks a cell")
	eq(TOUCH.drag_is_pan(Vector2.ZERO, Vector2(TOUCH.PAN_SLOP, 0), false), true, "a longer walk drag pans the map")
	eq(TOUCH.drag_is_pan(Vector2.ZERO, Vector2(80, 0), true), false, "a spell drag still aims")
	var tall_band := TOUCH.play_band_for(Vector2(960, 1400), true)
	eq(tall_band.y > tall_band.x, true, "a tall window still has a play band")
	var tall_zoom := TOUCH.board_zoom(960.0, 500.0, Vector2(960, 1400), true)
	eq(tall_zoom < 1.8, true, "a tall window does not cover-zoom to a few cells")
	var taller := TOUCH.board_zoom(960.0, 500.0, Vector2(960, 2000), true)
	near(taller, tall_zoom, "extra portrait height does not zoom further in")
	eq(taller < TOUCH.MOBILE_BOARD_ZOOM_MAX, true, "a 2000px window stays under the cap")
	eq(TOUCH.board_zoom(960.0, 500.0, Vector2(2400, 1600), true), TOUCH.MOBILE_BOARD_ZOOM_MAX, "an extreme window hits the zoom cap")
	var pawn := Pawn.new()
...
func _test_sources_keep_desktop_and_hub() -> void:
	...
	truthy(view.contains("drag_is_pan"), "a walk drag can pan the phone camera")
	truthy(view.contains("step_travel"), "walk steps use the planted stride")
	truthy(project.contains("window/handheld/orientation=4"), "phones stay in landscape")
	eq(project.contains("window/stretch/aspect=\"keep\""), false, "stretch does not letterbox with keep")
	var touch_src := FileAccess.get_file_as_string("res://ui/touch_adapter.gd")
	truthy(touch_src.contains("SCREEN_SENSOR_LANDSCAPE"), "runtime lock is sensor landscape")
	truthy(touch_src.contains("CONTENT_SCALE_ASPECT_EXPAND"), "runtime stretch fills the window")
	truthy(hub.contains("_TOUCH.lock_landscape_frame"), "hub locks landscape before it shows")
	truthy(view.contains("lock_landscape_frame"), "combat locks landscape before the board shows")
