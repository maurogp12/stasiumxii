...
func _test_mobile_target_pick() -> void:
	...
	near(portrait, TOUCH.MOBILE_CELL_TARGET_PX / TOUCH.DIAMOND_H, "portrait diamond is a 44px tap")
	eq(portrait <= TOUCH.MOBILE_BOARD_ZOOM_MAX, true, "portrait zoom stays inside the mobile cap")
	var shown_w := 960.0 / portrait / 960.0
	eq(shown_w >= TOUCH.MOBILE_MIN_VISIBLE, true, "portrait still shows most of the board width")
	var play_h := 1400.0 - TOUCH.PLAY_TOP - (TOUCH.VIEW_H - TOUCH.PLAY_BOTTOM)
	eq(500.0 * portrait <= play_h + 0.01, true, "portrait board height stays inside the clear band")
	var room := TOUCH.pan_room(960.0, 500.0, Vector2(960, 1400), portrait, true)
	eq(room.x > 40.0, true, "the raised phone zoom leaves sideways pan room")
	eq(room.y, 0.0, "portrait keeps the full board height in frame")
	...
	near(wide, 0.64, "a short mobile window does not slide the board under the chrome")
	var pawn := Pawn.new()
