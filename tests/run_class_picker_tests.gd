...
func _test_hotseat_p1_then_p2() -> void:
	...
	eq(picker.phase_name(), "map", "hot-seat starts at the map picker")
	eq(picker.prompt_text(), "Pick a Koliseo map", "map prompt names the Koliseo")
	eq(picker.map_cards_visible(), true, "five map cards are visible")
	var early_class: Dictionary = picker.pick_class("mender")
	eq(bool(early_class.get("ok", true)), false, "a class before a map does not pick")
	eq(str(early_class.get("reason", "")), "map_required", "map is required before classes")
	var picked_map: Dictionary = picker.pick_map("crosshaven")
	eq(bool(picked_map.get("ok", false)), true, "Crosshaven is accepted")
	eq(picker.phase_name(), "hotseat_p1", "hot-seat starts at P1 after the map")
