...
func _test_adapter_defaults_flat_ground() -> void:
	...
	eq(tiles.size(), 144, "adapter fills a 12x12 when snapshot has no tiles")
...
func _test_live_snapshot_tiles_from_combatsim() -> void:
	...
	eq(snap["board_size"], 12, "live chrome snap is 12×12")
	eq(SNAPSHOT_TILES.terrain_at(snap, Vector2i(0, 0)), "ground", "Mauro (0,0) is ground")
	eq(SNAPSHOT_TILES.elevation_at(snap, Vector2i(0, 0)), 0, "Mauro (0,0) elevation is the token")
	eq(SNAPSHOT_TILES.terrain_at(snap, Vector2i(1, 2)), "mud", "Mauro mud reaches chrome adapter")
	eq(SNAPSHOT_TILES.terrain_at(snap, Vector2i(0, 2)), "water", "Mauro water reaches chrome adapter")
	eq(SNAPSHOT_TILES.elevation_at(snap, Vector2i(7, 10)), 1, "Mauro token elevation reaches chrome")
...
func _test_demo_map_paints_from_snapshot() -> void:
	...
	var snap: Dictionary = _sim.snapshot()
	eq(str(snap.get("demo_map", "")), "mauro_12", "live chrome snap stamps the Mauro grid")
	eq(int(snap.get("board_size", 0)), 12, "live chrome board is 12×12")
	var mud := TILE_SCRIPT.new()
	mud.apply_board_data(SNAPSHOT_TILES.terrain_at(snap, Vector2i(1, 2)), SNAPSHOT_TILES.elevation_at(snap, Vector2i(1, 2)))
	eq(mud.terrain_letter(), "M", "Godot paints M on Mauro mud (1,2)")
	eq(mud.elevation_text(), "2", "Mauro mud elevation text is the token")
	mud.free()
	var water := TILE_SCRIPT.new()
	water.apply_board_data(SNAPSHOT_TILES.terrain_at(snap, Vector2i(0, 2)), SNAPSHOT_TILES.elevation_at(snap, Vector2i(0, 2)))
	eq(water.terrain_letter(), "W", "Godot paints W on Mauro water (0,2)")
	water.free()
	var ridge := TILE_SCRIPT.new()
	ridge.apply_board_data(SNAPSHOT_TILES.terrain_at(snap, Vector2i(7, 10)), SNAPSHOT_TILES.elevation_at(snap, Vector2i(7, 10)))
	eq(ridge.terrain_letter(), "G", "Godot paints G on Mauro ground (7,10)")
	eq(ridge.elevation_text(), "1", "Mauro ground elevation text is the token")
	ridge.free()
