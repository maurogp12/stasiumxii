...
func _test_live_snapshot_tiles_from_combatsim() -> void:
	...
	eq(SNAPSHOT_TILES.terrain_at(snap, Vector2i(9, 2)), "water", "Crosshaven water reaches chrome adapter")
	eq(SNAPSHOT_TILES.elevation_at(snap, Vector2i(5, 4)), 2, "Crosshaven ridge elevation is the tag")
...
func _test_demo_map_paints_from_snapshot() -> void:
	...
	var snap: Dictionary = _sim.snapshot()
	eq(str(snap.get("demo_map", "")), "crosshaven_12", "live chrome snap stamps Crosshaven")
	...
	water.apply_board_data(SNAPSHOT_TILES.terrain_at(snap, Vector2i(9, 2)), SNAPSHOT_TILES.elevation_at(snap, Vector2i(9, 2)))
	eq(water.terrain_letter(), "W", "Godot paints W on Crosshaven water (9,2)")
	water.free()
	var ridge := TILE_SCRIPT.new()
	ridge.apply_board_data(SNAPSHOT_TILES.terrain_at(snap, Vector2i(5, 4)), SNAPSHOT_TILES.elevation_at(snap, Vector2i(5, 4)))
	eq(ridge.terrain_letter(), "G", "Godot paints G on Crosshaven ground (5,4)")
