class_name ElevationCost
extends RefCounted

## Phase B+ elevation step helper. Proposed (not Locked).
## Uphill: +1 MP per full elevation level; a half-level climb counts as +1.
## Downhill: +0 MP.
## Max climb: 1.0. Max drop: 2.0.
## This is the gameplay elevation source. Z-sort is a separate VIEW function.
...
static func analyze(from_elev: float, to_elev: float) -> Dictionary:
	...
	if -delta > MAX_DROP + EPS:
		...
		return out
	if delta > EPS:
		# Proposed: +1 per full level; half-level (0.5) also +1.
		# ceil so a legal micro-climb is not invented as 0 MP.
