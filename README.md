...
Press **F5 / Play**. `run/main_scene` is `scenes/mobile_hub.tscn`. The hub on this branch has one door, **Koliseo**, which opens `scenes/class_select.tscn`. Stasis doors are not here.
...
- Advance: dest-click teleport, **3 AP / 0 MP**, instant snap. Range gate is **exactly 2 cardinal spaces** (N/S/E/W at Manhattan 2). Manhattan 1, diagonals, and any non-cardinal are rejected. Dest must pass the **same stand-on gates as walk** (walkable, not occupied, not lava, climb≤1 / drop≤2 via the shared `WalkBoard.stand_on_gate` helper). Gate only — no terrain+elev MP spend. Illegal dest refunds; `legal_intents` / `preview_cast` reflect the gates. Ironjaw-only. No MP spend (`submit` does not zero leftover MP). After Advance, leftover MP still walks. **Facing unchanged** (does not auto-face). Client path ignored. After resolve, client clears Advance so walk chrome returns while MP>0. Walk / Esc cancel Advance aim (right-click still faces).
- Spell range stays Chebyshev for Strike / Mark Shot / Detonate / Shoulder / Crush. Advance range is exactly 2 cardinal spaces (N/S/E/W at Manhattan 2).
...
| A06 | Advance dest-click teleport, exactly 2 cardinal spaces (N/S/E/W at Manhattan 2). Manhattan 1, diagonals, and any non-cardinal are rejected. 3 AP / 0 MP, instant snap. Dest uses the same stand-on gates as walk (walkable / occupied / lava / climb≤1 / drop≤2). Gate only — no MP spend. `submit` does not zero leftover MP; leftover MP still walks (`legal_intents` is mp>0, not AP). Adjacency = Chebyshev 1 after landing. Facing unchanged (Advance does not auto-face). |
...
Allowlist: `kestrel` | `ironjaw` | `mender` | `gloam` | `bastion`. The dedicated host is not a fighter and does not boot the default pair. Each player confirms a class, then joins the queue. Any two Locked classes pair. Seat 0 is the first queued session unless that session is bound to the other transport seat. Kits follow the chosen `class_id` (two of the same class is legal). Advance stays **3 AP / 0 MP**, exactly 2 cardinal spaces. Hot-seat and the listen-host buttons still start Kestrel vs Ironjaw.

Mender, Gloam, and Bastion use workbook v0.6 (`data/select_class_lock_kits_v0.6.json`). Proto is **80 HP / Mastery 0 / Resist 0** with the Locked combat refill (**6 AP / 3 MP**). Umbral is 0–4. Shades max 2. Ambush is **4 AP / 0 MP**, Manhattan **1–2 cardinal** from the origin (Gloam while Invisible, otherwise a live Shade). A Shade may be that origin only after the opponent has completed one full turn since the Drop; Fade's Invisible self-origin does not wait. The landing is the empty tile one step past the enemy on that axis. Ambush miss does not teleport, keeps Shade and Invisible, and spends 4 AP. An occupied back tile rejects and refunds. The Ambush button stays soft-grey until the cast is in `legal_intents`. Aegis Break hit clears all Aegis; a miss spends 0. Snap Wall is one blocked tile for 2 turns (walk, and a future Gust) only while a Bastion is in the match. Nightfold and the other `open_can_wait` edges reject instead of guessing.
...
4. End Turn. Hover **Advance**: exactly-2-cardinal teleport, no HIT %, no sample. **Shoulder** passes through Director Locked Shoulder (occupied `push_blocked`; OOB / truly blocked bounce + stagger for +2 Impact; lava land applies Burn). **Crush** at 4 Impact shows **Stun 1 (Locked A′) this cast**; at 0–2 Impact that flag stays off.
...
2. **Cancel checklist:** Select **Advance** (purple tiles on the four cardinal-2 spaces, walk chrome off). Press **Walk** or **Esc** — selection returns to Walk, cyan walk tiles come back. Right-click a tile still faces (does not cancel).
3. Select **Advance** again and dest-click a tile exactly 2 spaces N/S/E/W. The pawn snaps (no hops). Manhattan 1 and diagonal clicks are rejected. Advance deselects; walk tiles highlight from remaining MP. With e.g. 3 AP / 3 MP (or a second Advance to 0 AP / 3 MP), click a walk dest — it must be selectable and highlight.
...
5. Advance onto a 2-cardinal tile: position snaps, **facing is unchanged**. A Manhattan 1 or diagonal dest does not move the pawn. Walk last-hop facing still applies after. No HIT % on Advance or walks.
