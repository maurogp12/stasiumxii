...
5. Click a highlighted empty tile to **walk**. Dest-click only: `CombatSim` expands the cheapest orthogonal path (weighted pathfinder). Walk cost is dest **terrain MP + uphill integer z**; downhill is free. Legal tiles come from remaining MP. Live `reset_match` seeds **Crosshaven 12×12** from the tags JSON (terrain + elevation). `paint_only` / Tiled `props_paint` are paint only. The board paints those isometric PNGs on the 64×32 diamonds (`cell_to_local` ≈ `((x-y)*32, (x+y)*16)`). Proto `board_size` 8 still uses the 8×8 crop (origin row 2, col 2 — see below) plus seeded noise. The camera zooms the 12×12 diamond into 960×720 and middle-mouse pans. The pawn animates one ortho tile at a time along the returned path and **faces each hop** (final facing = last hop). Manual **Face** still turns in place (0 AP). The client never sends `intent.path`. Walk is the default mode. After selecting a spell, press **Walk** or **Esc** to cancel back to walk chrome (right-click still faces; it does not cancel). Hit % / facing cones / spell LoS ignore height.
...
9. While aiming **Mark Shot / Strike / Detonate / Shoulder / Crush**, the HUD shows the Locked **HIT %** for the current Chebyshev distance before you click (hover updates the cell). Advance (no roll) and walks never show hit %. Bands are Locked through Chebyshev 8: 1→90%, 2–3→80%, 4–5→75%, 6–8→70%. **Chebyshev 9+ is Open** — the cast is rejected and refunded, and the HUD shows no percent. Do not invent a hit % for 9+. A cast outside the spell’s max range stays illegal. Kit damage and ranges are unchanged.
...
11. First combatant to 0 HP loses. **New Match** resets from `CombatSim.reset_match()` (live path starts in DEPLOYMENT again) with a **new seed**. Ship elevation stays the Crosshaven tags. Proto `board_size` 8 still regenerates seeded noise on the same crop terrain.

## Architecture

| Piece | Role |
| --- | --- |
| `backend/combat_sim.gd` (autoload `CombatSim`) | Sole authority (including the 30s turn clock). `BOARD_SIZE` is **12**. `reset_match(config)` starts **DEPLOYMENT** (live) and seeds **Crosshaven 12×12 tags** (terrain + elevation) onto `WalkBoard`. `paint_only` is stored beside the walk tile. `MatchConfig.board_size` 8 is the proto crop + seeded noise. `MatchConfig.seed` / `elev_seed` override for fixtures/replay (`snapshot.seed`, `snapshot.elev_seed`). `place_unit(seat, cell)`, `ready_seat(seat)`, `can_place`, `can_ready`, `legal_deploy_cells(seat)`, `deploy_zone_cells(seat)`. `submit(intent)`, `legal_intents(seat)`, `snapshot()`, `aim_hit_preview(seat, spell, dest?)`, `preview_cast(...)`. `hit_chance` reads `backend/hit_bands.gd` (Locked through Chebyshev 8; 9+ is open and rejects with no percent). Walk is dest-click weighted pathfinder (`backend/walk_board.gd`); Advance reuses `stand_on_gate`. `snapshot().tiles` exposes per-tile **integer** elevation / terrain_type for Godot. Both Ready → lock → Turn 1 with spawn cells from confirmed positions. `skip_deploy` / `kestrel_pos` / `ironjaw_pos` skip to combat (tests/setup; same Crosshaven tags unless `flat_board` or an explicit proto size). Rolls and HP live here. |
| `backend/match_flow.gd` (`MatchFlow`, owned by CombatSim) | Locked phase + simultaneous ready. Proposed (shipped live) seed-based ~6-cell blob sampler on the **12×12** ship board; `legal_deploy_cells` / `deploy_zone_cells` come from those blobs. Owns `PHASE_A_DEMO_TILES` / `phase_a_demo_tiles()` — proto-only 8×8 **terrain** crop of Mauro’s 12×12 at origin (row 2, col 2) — and `generate_noise_elevations(seed)` for z 0–3. Ship matches load `art/maps/arena_colosseum_v2/tiled/crosshaven_12x12_tags.json`. #31 border halves stay the previous Locked baseline. Proto stays reference. |
...
- **12×12** ship board (`BOARD_SIZE`), per-tile **integer elevation** + **terrain_type** from Crosshaven tags. `paint_only` never affects pathing, LoS, or MP. Proto `board_size` 8 keeps the 8×8 crop of Mauro’s 12×12 (origin row 2, col 2; all 64 cells) plus **seeded noise** z 0–3. No invented walls. Spell LoS stays unused (no height mods).
...
- Hit bands (Locked, `backend/hit_bands.gd`): 1=90%, 2–3=80%, 4–5=75%, 6–8=70%. Chebyshev **9+ is Open**: the cast is rejected and refunded, and no hit % is invented. Out of spell max range stays illegal. Kit damage and ranges are unchanged.
...
Godot chrome on `main.tscn` paints snapshot terrain + elevation. **CombatSim owns walk costs** ([#36](https://github.com/maurogp12/stasiumxii/pull/36) weighted pathfinder). `snapshot().tiles` is the live payload — `reset_match` seeds **Crosshaven 12×12 tags** (see below). The view paints the Tiled PNGs on those diamonds. A `Camera2D` zooms that diamond into the 960×720 play band; middle-mouse pans. Walk dests are whatever `legal_intents` returns. Do **not** invent height→hit / facing / LoS.
...
**How to test live elevation chrome:** New Match on `main.tscn` shows the Crosshaven isometric tiles (not all Ground 0). A second New Match keeps the same terrain and the same tag z. After both Ready, cyan walk tiles are only sim-legal dests. The board fits 960×720 by camera zoom; middle-mouse pans. Face N/E/S/W and deploy chrome stay as they are. Headless: `godot --headless --path . -s res://tests/run_elevation_chrome_tests.gd`.

## Phase A demo map (proto `board_size` 8 — Director-stamped Locked 8×8 crop + seeded noise elev)

**Ship board is 12×12 Crosshaven.** This crop is proto-only (`MatchConfig.board_size` 8). Terrain is fixed in `MatchFlow.phase_a_demo_tiles()` (`PHASE_A_DEMO_TILES`) and applied when `board_size` is 8 (same terrain on `skip_deploy` unless `flat_board`). **Elevation is not the crop z.** Each proto reset generates z from `seed` (override with `MatchConfig.seed` / `elev_seed` for fixtures and replay). Smooth value noise → integer z 0–3 (z1–z4), preferring contiguous neighbors and avoiding checkerboard 0/3. `snapshot().tiles` exposes terrain + elevation **ints** on all 64 cells; `snapshot.elev_seed` stores the seed. Deploy still rejects lava (`not_walkable`); no elevation MP on place. No height→hit / facing / LoS. Ship matches ignore this crop and load `art/maps/arena_colosseum_v2/tiled/crosshaven_12x12_tags.json` (sibling `.tmx` is isometric art, not a blocker source).
...
1. Run the main scene. Select **Mark Shot**: Chebyshev 2–5 ring + **HIT 75%** vs default Ironjaw (range 5). Hover another in-range cell to see that cell’s Locked % (through 70% at dist 6–8). Walk chrome stays off. Advance / Walk never show HIT %. Chebyshev 9+ shows no percent and the cast is rejected.
