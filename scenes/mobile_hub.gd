...
## Entry (`project.godot` `run/main_scene`).
## Koliseo is the only door: PvP into the five existing boards (class select,
## then a random hot-seat arena). Stasis doors and dungeon scenes stay on the
## `mobile` branch. They are not wired here.
...
func _add_door(parent: Node, door_id: String, label: String) -> void:
	...
	button.add_theme_stylebox_override("normal", _door_style(false))
	button.add_theme_stylebox_override("hover", _door_style(true))
	button.add_theme_stylebox_override("pressed", _door_style(true))
	button.add_theme_stylebox_override("focus", _door_style(true))
