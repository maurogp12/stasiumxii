...
static func class_element(class_id: String) -> String:
	var labeled := class_element_text(class_id)
	if labeled != "":
		return labeled
...
static func class_resources(class_id: String) -> Array:
	if CLASS_RESOURCES.has(class_id):
		return CLASS_RESOURCES[class_id]
	return []


static func class_proto(class_id: String) -> Dictionary:
	if CLASS_PROTO.has(class_id):
		return CLASS_PROTO[class_id]
	return {}


static func class_passive(class_id: String) -> Dictionary:
	if CLASS_PASSIVES.has(class_id):
		return CLASS_PASSIVES[class_id]
	return {}
