...
- **Walk strip for this facing, and the clip is playing:** loop for the whole path. The pawn faces the step, and the cycle plays through a two-frame plant, before it slides through cell centers (about 0.30s each). `walk_strip_speed_scale` stretches one authored plant onto that tile. The snapshot facing snaps after the land. The sprite root takes a 4–6px gait (press, push-off, modest rise, settle). No tile-tall hop. Scale stays at rest while the cycle plays. Gloam uses `gloam_walk_*`.
