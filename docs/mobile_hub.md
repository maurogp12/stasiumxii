...
Banner art is the Koliseo crop in `art/ui/hub/koliseo_banner.png`. Each RAID plate (`raid_crosshaven.png` and the other four `raid_*.png`) is the same portrait, recomposed so the boss sits inside the gold cuadro with headroom above the name bar. The hub draws the full uppercase name (`CROSSHAVEN STASIS` through `STORMSPIRE STASIS`) in Cinzel on that bar, so the letters stay sharp when the tile scales. Each control's `custom_minimum_size.y` is 72. At 960×720 the tiles lay out about 121×169 with a gap between them, under the banner and clear of the footer rule, so the phone target stays fat. `window/handheld/orientation` is sensor landscape, so the phone shows this wide poster (title, banner, RAID, one horizontal row) instead of a tall portrait stack. A wider landscape window keeps that row aligned to the banner and pins the footer star to the bottom of the frame. Plates use keep-aspect centered, so the nameplates are not cropped.
