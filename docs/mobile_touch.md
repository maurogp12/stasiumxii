...
A legal walk eases the foot along the segment for about 0.30s (`Pawn.WALK_TILE_SEC`). `grid_position` is the tactical cell and updates when the foot commits. `pawn.position` is the visual foot on the diamond. The body faces each segment before that foot moves, and the matching `walk_n/e/s/w` strip is sampled from the step: the contact frame only while the foot is planted, passing frames only while it is between cells. During the stride the body leads about 10px along the facing and rises a few pixels, then both return so arrival is the idle facing on the cell. The contact shadow is the Foot child and stays on the ground.
