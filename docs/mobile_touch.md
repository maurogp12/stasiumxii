...
The 960×720 camera fit is unchanged (15×15 stays zoom 0.64 inside `PLAY_TOP` 140, `PLAY_BOTTOM` 460). Phones open in portrait. The combat camera zooms in until the iso diamond covers that clear band, so cells are large and the dark margin around the board is gone. The same 260px bottom reserve stays the thumb cluster. A walk-mode drag pans inside the board. While a unit spell is armed, the selected fighter pulses a ring. That ring is chrome only.
...
A legal walk is one continuous slide along cell centers (`Pawn.WALK_TILE_SEC`, about 0.30s per tile, straight and diagonal alike). `grid_position` is the tactical cell and updates when the foot commits. `pawn.position` is the visual foot and slides between those cells. Playback puts the body back on the departure tile first, so a snapshot refresh cannot skip the travel. Facing turns in place for two walk frames, then the body follows that cardinal segment. The walk strip's authored plant (half of the 6-frame cycle) is stretched onto that 0.30s tile (`Pawn.walk_strip_speed_scale`). The sprite root and the name take a 4–6px gait: a press into the tile, a push-off, a modest rise, and a landing settle (`ViewMotion.HOP_PX`). The contact shadow is the Foot child and stays on the ground, shrinking while the body is up. Scale does not squash or stretch while a walk strip is playing. A missing strip, or a `play()` that does not start, keeps that same gait on the static facing and adds the fallback squash. Drop Shade's token is a board marker (`board/shade_marker.gd`), not a shader pool. Advance stays a teleport snap and does not bounce.
