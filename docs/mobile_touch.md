...
A legal walk is one continuous slide along cell centers (`Pawn.WALK_TILE_SEC`, about 0.22s per tile, straight and diagonal alike). Playback puts the body back on the departure tile first, so a snapshot refresh cannot skip the travel. Facing snaps at each cell. There are no turn frames and no pause between cells. The walk strip loops at its authored fps for the whole path. The sprite root and the name take a 4–6px step bounce (`ViewMotion.WALK_BOUNCE_PX`, was `HOP_PX` 36) on the walk cycle (`ViewMotion.WALK_STEP_SEC`). The seat ring stays on the tile. Scale does not squash or stretch. A missing strip, or a `play()` that does not start, keeps that same bounce on the static facing. Drop Shade's token is a board marker (`board/shade_marker.gd`), not a shader pool. Advance stays a teleport snap and does not bounce.
