...
The 960×720 camera fit is unchanged (15×15 stays zoom 0.64 inside `PLAY_TOP` 140, `PLAY_BOTTOM` 460). Phones stay landscape (`sensor landscape`). The combat camera shows most of the iso diamond: on a 20:9 window (canvas about 1600×720) the zoom is about 1.48, so a diamond is about 47px tall and the width of the board still fits. The turn plaque and the thumb cluster overlay the edges. A walk-mode drag pans inside the board. While a unit spell is armed, the selected fighter pulses a ring. That ring is chrome only.
