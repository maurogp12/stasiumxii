...
The 960×720 camera fit is unchanged (15×15 stays zoom 0.64 inside `PLAY_TOP` 140, `PLAY_BOTTOM` 460). On a phone portrait the viewport is taller than 720. That extra height goes to the board, and the same 260px bottom reserve stays clear of the thumb cluster. While a unit spell is armed, the selected fighter pulses a ring. That ring is chrome only.
