...
Everything that ships on the **mobile** track **merges up to PC `main`**, **except dungeons**. Luca unparked Stasis for the phone APK overnight 2026-09-25 (PR #132). That work stays on `mobile`. This tree does not include those scenes, the catalog, the foe AI, the dungeon tests, or the provisional foe numbers.
...
F5 → `scenes/mobile_hub.tscn` with one door: **Koliseo** → `scenes/class_select.tscn`. Stasis doors stay on the `mobile` branch.
...
- **Gender + color cosmetics:** **Parked** (Studio backlog). Dungeons stay on the `mobile` branch.
...
- Random Koliseo map selection; Koliseo-only hub on PC `main`. Stasis doors stay on `mobile`.
...
- **Dungeons / Stasis rooms** — on the `mobile` branch only (Luca, overnight 2026-09-25, PR #132). Not in this tree. No loot or keys in that slice. Provisional foe numbers were not copied here. Gender/color stays parked.
...
7. Current priority stack: (1) walk + action feel + Batch-1c, (2) spell chrome readability, (3) dungeons stay on the `mobile` branch, (4) cosmetics later.
